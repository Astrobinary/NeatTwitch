...
class videoplayerlist extends Component {
	constructor(props) {
		...
		if (this.props.location.state === undefined) {
			...
			this.state = { src: "", video: this.props.location.state.videos[this.props.location.state.current], next: this.props.location.state.videos[this.props.location.state.next], prev: this.props.location.state.videos[this.props.location.state.prev] };
		}
	}
	...
	updateVideos = next => {
		if (next) {
			this.setState({ video: this.props.location.state.videos[this.props.location.state.current + 1], next: this.props.location.state.videos[this.props.location.state.next + 1], prev: this.props.location.state.videos[this.props.location.state.prev + 1] });
		} else {
			this.setState({ video: this.props.location.state.videos[this.props.location.state.current - 1], next: this.props.location.state.videos[this.props.location.state.next - 1], prev: this.props.location.state.videos[this.props.location.state.prev - 1] });
		}
	};
	...
	render() {
		...
		return (
			<section className="videoplayer">
				<SimpleStorage parent={this} blacklist={["src", "height", "video", "next", "prev"]} />
